...
public class ConfigurationManager {
	...
	/**
	 * Sets the value of the specified property and saves it to the config file.
	 * @param keyName the key of the property to set
	 * @param value the value to assign to the property
	 */
	public static void setProperty(String keyName, String value) {
		try (FileOutputStream output = new FileOutputStream(
				System.getProperty("user.dir") + "/src/main/resources/config.properties")) {
			CONFIG_FILE.setProperty(keyName, value);
			CONFIG_FILE.store(output, "Updated property: " + keyName);
		}
		catch (IOException e) {
			e.printStackTrace();
